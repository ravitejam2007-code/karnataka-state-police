...
import { 
  ...
  Shield, Radio, FileSearch, LineChart, Building2, User, KeyRound
} from "lucide-react"
import { useTranslation } from "react-i18next"
import { toast } from "sonner"

import { useAuthStore, type Role } from "@/store/useAuthStore"
import { VerifyRolePinModal } from "@/features/settings/components/VerifyRolePinModal"
...
export function RoleSelection() {
  ...
  const [roleToVerify, setRoleToVerify] = useState<{ role: Role; title: string } | null>(null)
  ...
  const handleActivateRoleClick = (roleToActivate: Role) => {
    const meta = ROLE_METADATA[roleToActivate]
    setRoleToVerify({
      role: roleToActivate,
      title: meta?.title || roleToActivate,
    })
  }

  const performRoleActivation = (roleToActivate: Role) => {
    setActiveRole(roleToActivate)
    toast.success(`Active session role set to: ${roleToActivate}`, {
      description: "Role PIN verified. Access controls updated for current session.",
    })
  ...
  return (
    <>
      <Card className="w-full shadow-md border-t-4 border-t-[#0F172A] border-[#E2E8F0] bg-white font-sans max-w-md mx-auto rounded-xl">
        <CardHeader className="space-y-0.5 text-center px-4 sm:px-6 pb-2 pt-3 border-b border-[#F1F5F9]">
          ...
          <CardDescription className="text-[11px] text-[#475569]">
            Welcome, <span className="font-semibold text-[#0F172A]">{user.name}</span> ({user.department}). Select session role to verify PIN.
          </CardDescription>
        </CardHeader>
        ...
        <CardFooter className="flex items-center justify-between gap-2 border-t border-[#E2E8F0] py-2.5 px-4 sm:px-6 bg-[#F8FAFC] rounded-b-xl text-xs">
          ...
          <Button 
            onClick={() => handleActivateRoleClick(selectedRole)}
      ...
      {/* Verify Role PIN Modal */}
      {roleToVerify && (
        <VerifyRolePinModal
          isOpen={!!roleToVerify}
          onClose={() => setRoleToVerify(null)}
          role={roleToVerify.role}
          roleTitle={roleToVerify.title}
          onSuccess={() => {
            const roleKey = roleToVerify.role
            setRoleToVerify(null)
            performRoleActivation(roleKey)
          }}
        />
      )}
    </>
